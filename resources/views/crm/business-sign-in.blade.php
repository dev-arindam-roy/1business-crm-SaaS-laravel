    ...
    <body>
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-6">
                    <form name="frmx_signIn" id="frmx_signIn" action="{{ route('auth.businessLogin.Process', array('subdomain' => $subDomain->name)) }}" method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-md-12">
                        ...
                        @if (isset($emailVerifiedMsg) && $emailVerifiedMsg)
                        <div class="row">
                            <div class="col-md-12">
                                <div class="alert alert-info alert-dismissable">
                                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button>
                                    <p><strong>Congratulations</strong>, Your email address verified successfully, thankyou.</p>
                                </div>
                            </div>
                        </div>
                        @endif

                        @if (isset($subDomain) && !empty($subDomain) && isset($businessInfo) && !empty($businessInfo) && isset($ownerInfo) && $ownerInfo->status == 1)
                        <div class="row">
                            <div class="col-md-12">
                                <div class="note note-success">
                                    <p class="block font-green">Are you sure? Are you want to Sign-In with:</p>
                                    <p class="mt--1 font-green-seagreen"><i class="icon-briefcase"></i> <strong>{{ $businessInfo->business_name }}</strong></p>
                                </div>
                                <input type="hidden" name="subdomain" value="{{ $subDomain->name }}"> 
                            </div>
                        </div>
                        @endif

                        @if ((isset($ownerInfo) && $ownerInfo->status != 1) || (isset($subDomain) && $subDomain->status != 1))
                        <div class="row">
                            <div class="col-md-12">
                                <div class="note note-danger">
                                    <p class="block font-red-mint"><i class="icon-briefcase"></i> <strong>{{ $businessInfo->business_name }}</strong></p>
                                    <p class="mt--1 font-red-thunderbird">Is temporary Inactived or Blocked, Please contact with business owner.</p>
                                </div>
                            </div>
                        </div>
                        @endif
                        ...
                        @if (isset($ownerInfo) && $ownerInfo->status == 1)
                        <div class="row">
                            <div class="col-md-12">
